...
import { fileEntry } from "./lib/review_bundle.mjs";
...
export async function qaReport(deckDir, sourceDir, { checkpointRecord } = {}) {
  sourceDir = sourceDir || path.resolve(deckDir, "..");
  requireApprovedCheckpoint({
    rootDir: ROOT,
    checkpointRecord,
    artifactPath: path.join(sourceDir, "deck.json"),
...
if (isMain(import.meta.url, process.argv[1])) {
  let parsed;
  try {
    parsed = parseCheckpointArgument(process.argv.slice(2), "qa_report.mjs <deckDir> [sourceDir]");
  } catch (error) {
    console.error(`ERROR: ${error.message}`);
    process.exit(1);
  }
  const [deckDir, sourceDir, ...extra] = parsed.positional;
  if (!deckDir) { console.error("usage: qa_report.mjs <deckDir> [sourceDir] --checkpoint <record.json>"); process.exit(1); }
  if (extra.length) { console.error("ERROR: qa_report.mjs accepts only <deckDir> and optional [sourceDir]"); process.exit(1); }
  qaReport(deckDir, sourceDir, { checkpointRecord: parsed.checkpointRecord }).then((findings) => {
    const { text, p0, p1 } = summarize(findings);
    console.log(text);
    const resolvedSourceDir = sourceDir || path.resolve(deckDir, "..");
    fs.writeFileSync(
      path.join(deckDir, "qa_report.json"),
      JSON.stringify({
        schema_version: 1,
        kind: "scholar-slides-qa-report",
        deck: fileEntry(path.join(resolvedSourceDir, "deck.json"), "QA deck artifact"),
        findings,
      }, null, 2),
    );
