...
def _page_blocks(page):
    ...
    for b in page.get_text("dict").get("blocks", []):
        if b.get("type", 0) != 0:  # 0 = text block
            continue
        spans = [
            {"text": span.get("text", ""), "size": span.get("size")}
            for line in b.get("lines", []) for span in line.get("spans", [])
        ]
        text = " ".join(span["text"] for span in spans)
        if text.strip():
            # Span sizes are faithful PDF layout evidence used only for generic
            # first-page metadata extraction; retain the old block fields too.
            out.append({"text": text, "bbox": tuple(b["bbox"]), "spans": spans})
    return out
