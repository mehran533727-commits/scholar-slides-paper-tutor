...
function provenanceContent(s) {
  ...
  if (entries.length) {
    return entries.map((entry) =>
      `<span class="provenance-entry" data-provenance-role="${escapeHtml(entry.role || "source")}">` +
      `<span class="provenance-label">${escapeHtml(entry.label)}：</span>` +
      `<span class="cite">${escapeHtml(entry.source_ref)}</span></span>`
    ).join(" · ");
  }
  return s.source_ref ? `<span class="cite">${escapeHtml(s.source_ref)}</span>` : "";
}

function source(s) {
  const content = provenanceContent(s);
  return content ? `<div class="s-source">${content}</div>` : "";
}

function audienceFigure(s) {
  if (!s.figure || !Array.isArray(s.provenance_display?.entries) || !s.provenance_display.entries.length) {
    return s.figure;
  }
  return { ...s.figure, cite: null };
}
...
const L = {
  "paper-title": (s, ctx) =>
    section(
      ...
      `<h1>${renderText(s.title)}</h1>` +
        ...
        (s.figure ? `<div class="s-title-hero">${renderFigure(s.figure, ctx)}</div>` : "")
    ),
  ...
  "assertion-evidence": (s, ctx) => {
    ...
    const displayedFigure = audienceFigure(s);
    const fig = hero ? { ...displayedFigure, caption: null } : displayedFigure;
    let foot = source(s);
    if (hero && (s.figure.caption || s.source_ref)) {
      ...
      const cite = !hasProvenanceDisplay && s.figure.cite ? ` <span class="cite">[${escapeHtml(s.figure.cite)}]</span>` : "";
      const cap = s.figure.caption ? `${escapeHtml(s.figure.caption)}${cite}` : "";
      const ref = provenanceContent(s);
  ...
  "results-table": (s) =>
    section(
      "results-table",
      "s-results",
      titleBar(s) + `<div class="s-body">${s.points ? list(s.points) : ""}${renderTable(s.table)}</div>` + source(s)
    ),
  ...
  "two-column": (s, ctx) =>
    section(
      ...
      titleBar(s) +
        ...
        `<div class="col-fig">${s.figure ? renderFigure(audienceFigure(s), ctx) : list(s.points2 || [])}</div>` +
        `</div></div>` +
        source(s)
    ),
  ...
  "evidence-flow": (s) => {
    ...
    const labelLengths = nodes.map((node) => [...String(node?.label || "")].length);
    // Horizontal flows become projector-illegible before they overflow: once five
    // stages share the body band, technical labels are forced into character-level
    // wrapping. Four-stage flows with a substantial label have the same failure mode.
    const crowdedStageSequence = nodes.length >= 5 ||
      (nodes.length >= 4 && labelLengths.some((length) => length > 32));
    const stacked = crowdedStageSequence || labelLengths.some((length) => length > 220) ||
      labelLengths.reduce((total, length) => total + length, 0) > 360;
    const flowClass = `evidence-flow${stacked ? " evidence-flow--stacked" : ""}`;
    return section(
      "evidence-flow",
      "s-evidence-flow",
      titleBar(s) +
        `<div class="s-body"><div class="${flowClass}">${nodes.map((node, index) =>
          `<div class="evidence-node">${renderText(node.label || "")}</div>` +
          (index + 1 < nodes.length ? `<div class="evidence-arrow" aria-hidden="true">→</div>` : "")
        ).join("")}</div></div>` + source(s)
    );
  },
...
const REQUIRED = {
  ...
  equation: ["equations"], "results-table": ["table"], "critique-concerns": ["points"], "evidence-flow": ["native_diagram"],
...
// Layouts that present data from the paper must carry a provenance reference before export.
const FACTUAL = new Set(["assertion-evidence", "results-table", "equation", "two-column", "evidence-flow"]);
...
export function validateSpec(deck) {
  ...
  (deck?.slides || []).forEach((s, i) => {
    ...
    if (FACTUAL.has(s.layout) && isEmpty(s.source_ref)) {
      problems.push({ slide, layout: s.layout, field: "source_ref", severity: "error",
        detail: `${s.layout} slide ${slide} has no source_ref — provenance is required on a factual slide` });
    }
