...
export const FIGURE_LEGIBILITY_THRESHOLDS = Object.freeze({
  minProjectedPx: 12,
  compressScale: 0.45,
  minNaturalW: 1600,
  arTolerance: 0.2,
});
export const VOID_THRESHOLDS = Object.freeze({ vGap: 400, hFrac: 0.62 });
...
const VOID_EXEMPT = new Set(["paper-title", "section", "equation", "references"]);
 ...
 */
export function voidFindings(slide, { vGap = VOID_THRESHOLDS.vGap, hFrac = VOID_THRESHOLDS.hFrac } = {}) {
  const out = [];
  ...
  if (gap > vGap) {
    out.push({ check: "vertical-void", severity: "P3",
      ...
      measurement: { bottom_gap_px: Math.round(gap), body_bottom_px: Math.round(slide.bodyBottom) },
      threshold: { max_bottom_gap_px: vGap },
      action: "rebalance the slide or move detail up from speaker notes" });
  }
  if (slide.bodyRight < hFrac * STAGE.w) {
    out.push({ check: "horizontal-void", severity: "P3",
      ...
      measurement: { body_right_px: Math.round(slide.bodyRight), stage_width_px: STAGE.w },
      threshold: { min_body_right_px: hFrac * STAGE.w },
      action: "use the full canvas" });
  }
  return out;
}
...
export function figureFindings(imgs, figMeta = new Map(), {
  minProjectedPx = FIGURE_LEGIBILITY_THRESHOLDS.minProjectedPx,
  compressScale = FIGURE_LEGIBILITY_THRESHOLDS.compressScale,
  minNaturalW = FIGURE_LEGIBILITY_THRESHOLDS.minNaturalW,
  arTolerance = FIGURE_LEGIBILITY_THRESHOLDS.arTolerance } = {}) {
  const out = [];
    ...
    if (meta?.minFontPt && cropMatchesBbox) {
      ...
      if (projected < minProjectedPx) {
        out.push({ check: "figure-text-illegible", severity: "P2",
          ...
          measurement: { bbox_width_pt: meta.bboxWpt, min_font_pt: meta.minFontPt, projected_text_px: Number(projected.toFixed(1)), rendered_width_px: im.rendered },
          threshold: { min_projected_text_px: minProjectedPx },
          action: "enlarge the figure, crop one panel, or redraw natively" });
      }
      ...
      out.push({ check: "figure-compressed", severity: "P2",
        ...
        measurement: { native_width_px: im.natural, rendered_width_px: im.rendered, rendered_scale: Number((im.rendered / im.natural).toFixed(4)) },
        threshold: { min_native_width_px: minNaturalW, min_rendered_scale: compressScale },
        action: "enlarge the figure, crop one panel, or redraw natively" });
    }
  }
  return out;
...
/**
 * Turn renderer diagnostics into the source-bound legibility contract consumed by CKPT-2
 * visual QA.  The underlying geometry remains in figureFindings/voidFindings above.
 */
export function structuredLegibilityFindings({ slides = [], figures = [] } = {}) {
  const figMeta = buildFigMeta(figures);
  const out = [];
  for (const slide of slides) {
    if (!slide || !Number.isInteger(slide.slide)) continue;
    for (const finding of voidFindings({ layout: slide.layout, ...(slide.body || {}) })) {
      out.push({
        slide: slide.slide,
        asset: null,
        source_locator: null,
    ...
    for (const image of slide.images || []) {
      for (const finding of figureFindings([image], figMeta)) {
        const locators = slide.assetLocators || {};
        out.push({
          slide: slide.slide,
          asset: image.src || null,
          source_locator: locators[image.src] || locators[cropKey(image.src)] || null,
          measurement: finding.measurement,
          threshold: finding.threshold,
          check: finding.check,
          severity: finding.severity,
