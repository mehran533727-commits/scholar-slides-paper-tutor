...
export function notesHandout(deck, opts = {}) {
  ...
  (deck.slides || []).forEach((s, i) => {
    ...
    const refs = s.speaker_content?.source_refs;
    const structuredTexts = (Array.isArray(refs) ? refs : []).map((ref) => {
      const page = Number.isInteger(ref.source_page) ? `p. ${ref.source_page}` : "source";
      const section = ref.section ? ` — ${ref.section}` : "";
      const locator = ref.locator ? ` — ${ref.locator}` : "";
      return `${page}${section}${locator}`;
    });
    const deckSegments = String(s.source_ref || "").split(/[；;]/).map((value) => value.trim()).filter(Boolean);
    const seen = new Set(structuredTexts.map((value) => value.replace(/\s+/g, "")));
    const extra = deckSegments.filter((segment) => {
      const key = segment.replace(/\s+/g, "");
      if (seen.has(key)) return false;
      seen.add(key);
      return true;
    });
    const allRefs = [...structuredTexts, ...extra];
    if (allRefs.length) {
      lines.push("", "**来源**");
      allRefs.forEach((text) => lines.push(`- ${text}`));
    }
    lines.push("");
  });
  return lines.join("\n");
