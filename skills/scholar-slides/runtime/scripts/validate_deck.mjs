...
export function validateDeck(deckDir) {
  ...
  const visibleMarkers = html.match(/\[(?:MISSING|UNVERIFIED)(?::[^\]]*)?\]/gi) || [];
  if (visibleMarkers.length) {
    findings.push({ check: "visible-marker", severity: "P1",
      detail: `renderer-visible unresolved marker(s): ${[...new Set(visibleMarkers)].join(", ")}` });
  }
  const auditTerms = html.match(/\b(?:CKPT[- ]?\d+|checkpoint|audit(?:ed)?|ledger|marker[ _-]?resolution|SHA[- ]?256|hash(?:ed|ing)?)\b/gi) || [];
  if (auditTerms.length) {
    findings.push({ check: "internal-audit-prose", severity: "P1",
      detail: `renderer-visible checkpoint or audit prose: ${[...new Set(auditTerms)].join(", ")}` });
    if (new Set(auditTerms.map((term) => term.toLowerCase())).size < auditTerms.length) {
      findings.push({ check: "duplicated-audit-prose", severity: "P1",
        detail: "checkpoint or audit prose is duplicated in renderer-visible HTML" });
    }
  }
  ...
  for (const src of extractImgSrcs(html)) {
    ...
    let resourcePath;
    try {
      resourcePath = resolveResourcePath(deckDir, src);
    } catch (error) {
      findings.push({ check: "unsafe-figure-path", severity: "P0", detail: error.message });
      continue;
    }
