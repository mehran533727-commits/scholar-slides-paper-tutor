...
from font_preflight import apply_cjk_font, detect_cjk_font

OKABE_ITO = ["#0072B2", "#D55E00", "#009E73", "#CC79A7", "#E69F00", "#56B4E9", "#000000"]
CJK_RE = re.compile(r"[\u3400-\u9fff]")


def required_cjk_text(spec) -> str:
    """Return every distinct Han character rendered from the chart spec, in stable order."""
    return "".join(dict.fromkeys(CJK_RE.findall(json.dumps(spec, ensure_ascii=False))))


def requires_cjk_font(spec) -> bool:
    """Detect Chinese text in a chart spec instead of silently drawing tofu glyphs."""
    return bool(required_cjk_text(spec))


def configure_cjk_font_if_needed(spec) -> str | None:
    if not requires_cjk_font(spec):
        return None
    family = detect_cjk_font(required_cjk_text(spec))
    if not family:
        raise RuntimeError(
            "Chinese chart export requires a CJK font with glyph coverage; "
            "install Noto Sans/Serif CJK, Source Han, Microsoft YaHei, or SimSun."
        )
    apply_cjk_font(plt.rcParams, family)
    return family
...
def render(spec, out_path, dpi=200):
    configure_cjk_font_if_needed(spec)
    d = chart_data(spec)
