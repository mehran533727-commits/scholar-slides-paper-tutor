...
import { normalizeResourceUri, resolveResourcePath } from "./lib/resource_path.mjs";
import { isMain } from "./lib/platform.mjs";
import { BUILD_BUNDLE_KIND, writeReviewBundle } from "./lib/review_bundle.mjs";
...
export function buildDeck(deckJsonPath, outDir, { checkpointRecord } = {}) {
  ...
  const figSrc = resolveResourcePath(
    path.dirname(deckJsonPath),
    normalizeResourceUri(deck.meta?.figures_dir || "figures"),
  );
  ...
  const buildManifestPath = writeReviewBundle({
    deckJsonPath,
    deckDir,
    outPath: path.join(deckDir, ".scholar-slides-build.json"),
    kind: BUILD_BUNDLE_KIND,
  });
  return { deckDir, htmlPath, printPath, buildManifestPath, nSlides: (deck.slides || []).length };
}

if (isMain(import.meta.url, process.argv[1])) {
  let parsed;
  try {
    parsed = parseCheckpointArgument(process.argv.slice(2), "build_deck.mjs <deck.json> [outDir]");
  } catch (error) {
    console.error(`ERROR: ${error.message}`);
    process.exit(1);
  }
  const [deckJson, outDir, ...extra] = parsed.positional;
  if (!deckJson) {
    console.error("usage: build_deck.mjs <deck.json> [outDir] --checkpoint <record.json>");
    process.exit(1);
  }
  if (extra.length) {
    console.error("ERROR: build_deck.mjs accepts only <deck.json> and optional [outDir]");
    process.exit(1);
  }
  try {
    const r = buildDeck(deckJson, outDir, { checkpointRecord: parsed.checkpointRecord });
